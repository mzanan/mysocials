  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:flex-row sm:justify-center sm:gap-8">
          ...
          {ACCENT_PRESETS.map((c) => (
            <button
              ...
              className={cn(
                ...
                accent === c
                  ? "ring-fg ring-offset-app-bg ring-2 ring-offset-2"
                  : "ring-hairline-strong ring-1 hover:scale-105"
              )}
      ...
      <div className="flex w-full max-w-xs flex-col gap-2.5">
        <span
          aria-hidden
          className="hidden text-xs font-medium sm:invisible sm:block"
        >
          Accent color
        </span>
        <div className="relative">
          {showHint && (
            <div className="bg-fg text-app-bg absolute -top-2 left-1/2 z-10 -translate-x-1/2 -translate-y-full rounded-lg px-3 py-1.5 text-xs font-medium whitespace-nowrap shadow-lg">
              This is how your links look
              <span className="bg-fg absolute top-full left-1/2 size-2 -translate-x-1/2 -translate-y-1/2 rotate-45" />
            </div>
          )}
          <button
            type="button"
            onClick={dismissAccentHint}
            className="link-btn bg-surface text-fg flex h-12 w-full items-center gap-3 rounded-2xl border px-4 text-[15px] font-medium"
            style={{
              ["--accent-glow" as string]: accent,
              borderColor:
                "color-mix(in oklab, var(--accent-glow) 42%, transparent)",
              boxShadow:
          ...
          >
            <span
              ...
              style={{
                backgroundColor:
                  "color-mix(in oklab, var(--accent-glow) 16%, transparent)",
              }}
            >
