...
export function SocialIcon({ url, size = 32 }: { url: string; size?: number }) {
  ...
  return (
    <div
      ...
      style={{ background: BRAND_BG[brand], width: size, height: size }}
    >
      {Icon ? <Icon size={iconSize} color="white" strokeWidth={2} /> : <TikTokIcon size={iconSize} />}
    </div>
  )
}
